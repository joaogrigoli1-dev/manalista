...
import { TtsPlayer } from "@/components/a11y/TtsPlayer";
...
/** Serializa os resultados em texto plano para o TTS */
function buildTtsText(results: DiagnosticSuggestion[], childName: string, pt: boolean): string {
  const lines: string[] = [];

  lines.push(
    pt
      ? `Relatório de sugestão diagnóstica para ${childName}.`
      : `Diagnostic suggestion report for ${childName}.`
  );

  results.forEach((r, idx) => {
    if (results.length > 1) {
      lines.push(pt ? `Hipótese ${idx + 1}:` : `Hypothesis ${idx + 1}:`);
    }

    lines.push(
      pt
        ? `Patologia principal: ${r.primaryPathology}. Confiança: ${r.confidence}.`
        : `Primary pathology: ${r.primaryPathology}. Confidence: ${r.confidence}.`
    );

    if (r.treatmentSuggestions.length > 0) {
      lines.push(pt ? "Sugestões de tratamento:" : "Treatment suggestions:");
      r.treatmentSuggestions.forEach((t) => {
        lines.push(`${t.modality}: ${t.description}`);
      });
    }
  });
  ...
  lines.push(
    pt
      ? "Atenção: este relatório foi gerado por inteligência artificial em modo demonstração e não substitui avaliação profissional."
      : "Disclaimer: this report was generated by AI in demo mode and does not replace professional evaluation."
  );
...
export function DiagnosticReport({ results, lang, childName, downloadError }: DiagnosticReportProps) {
  ...
  const ttsText = buildTtsText(results, childName, pt);
  const ttsLang = pt ? "pt-BR" : "en-US";
    ...
    <div className="animate-in" style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{
        ...
        <TtsPlayer
          text={ttsText}
          lang={ttsLang}
          label={pt ? "Ouvir relatório diagnóstico" : "Listen to diagnostic report"}
          className="mt-2"
        />
